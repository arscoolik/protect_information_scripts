...
def chinese_remainder_theorem(c1, m1, c2, m2, filename):
    ...
    n = m1 * m2
    write_to_docx(f"4.Из китайской теоремы об остатках, считаем такую штуку :(m * aq * x + d * ap * y) % n" , filename)
...
def rsa_sign(n, d, m, filename): 
    write_to_docx(f"Условие: Подписать сообщение по схеме RSA. Закрытый ключ: n = {n}; d = {d}. Сообщение: m = {m}.", filename)
    write_to_docx("Зашифрованное сообщение s = m^d mod n", filename)
    write_to_docx("Для того, чтобы это найти вопспользуемся китайской теоремой об остатках", filename)
    p, q = divisors(n)
    write_to_docx(f"1.Найдем делители числа n. Делители числа n это {p}, {q}", filename)

    ans = pow_with_crt(m, d, p, q, filename)
